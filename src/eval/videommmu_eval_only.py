...
def extract_choice_letter(text: str) -> str:
    # VideoMMMU uses options up to J
    s = extract_answer(text)
    m = re.search(r"\b([A-J])\b", s, re.IGNORECASE)
    if m:
        return m.group(1).lower()
    m = re.search(r"([A-J])", s, re.IGNORECASE)
    if m:
        return m.group(1).lower()
    return norm(s)
...
def main() -> None:
    ...
    strict_tag_letter_re = re.compile(r"^\s*<ANSWER[S]?>\s*[A-J]\s*</ANSWER[S]?>\s*$", re.IGNORECASE | re.DOTALL)
    ...
    for row in rows:
        ...
        elif re.match(r"^\s*[A-J]\.", text, re.IGNORECASE):
            format_class = "plain_option_sentence"
        elif re.match(r"^\s*[A-J]\s*$", text, re.IGNORECASE):
